 ...
 * 1/26/23
 * Due: 2/10/2023
 * Purpose: In this assignment you will implement your OWN version of the String class and some of its 
	...
	*/
	...
	public char charAt(int index) {
		...
		if (index >= 0 && index < charArray.length) {
			...
			throw new IndexOutOfBoundsException("Try a new index in the range from 0-" + (charArray.length - 1) + " for " + this.myString);
		}
	...
	public MyString replace(char target, char replacement) {
		...
		for (int i = 0; i < this.charArray.length; i++) {
			if (this.charArray[i] == target) {
				this.charArray[i] = replacement;
			}
		}
	...
	public int compareTo(MyString anotherString) {
		...
		else if (this.charArray.length > anotherString.charArray.length) {
			return 1;
		}
		else {
			return -1;
