...
class DequeTest {
	...
	final Integer[] RVALUES = { 67, 85, -17, 42 };
	...
	void testToString() {
		...
		for (int i = 0; i < VALUES.length; i++) {
			deque.addLast(VALUES[i]);
			assertEquals(Arrays.toString(Arrays.copyOf(VALUES, i + 1)), deque.toString());
		}
	}
	...
	void testToStringReverse() {
		Deque<Integer> deque = new Deque<Integer>();
		assertEquals("[]", deque.toStringReverse());
		for (int i = 0; i < VALUES.length; i++) {
			deque.addLast(VALUES[i]);
			assertEquals(Arrays.toString(Arrays.copyOfRange(RVALUES, RVALUES.length - i - 1, RVALUES.length)), deque.toStringReverse());
		}
	}
}